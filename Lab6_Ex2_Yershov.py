...
s3.color = (0, 255, 255, 150)
s3.score = 0
s3.body.Q = [[0, 0], [0, 0], [0, 0]]
s3.body.action = 1  # 0 - залишати, 1 - змінювати (випадковий кут)
...
def strategy2(b=s3.body):
    ...
    ellipse(x, y, 400, 400, stroke=Color(0.5))
    line(x, y, x + 200 * math.cos(a + 0.5), y + 200 * math.sin(a + 0.5), stroke=Color(0.5))
    line(x, y, x + 200 * math.cos(a - 0.5), y + 200 * math.sin(a - 0.5), stroke=Color(0.5))

    if canvas.frame % 10 == 0:  # Оновлюємо кожні 10 кадрів
        inS = inSector(s1.body.position[0], s1.body.position[1], x, y, 200, a)  # М'яч в секторі
        inS2 = inSector(S2[0].body.position[0], S2[0].body.position[1], x, y, 50, a)  # Антиоб'єкт

        # Визначаємо стан і винагороду
        if inS:  # М'яч у секторі
            state = 1
            reward = 10 if b.action == 0 else -5  # Більша винагорода за правильну дію з м'ячем
